...
public sealed class GscDiagnosticsAnalyzer(GscIndexer indexer)
{
    ...
    public async Task<List<Diagnostic>> CollectDiagnosticsAsync(string filePath, string text, CancellationToken cancellationToken)
    {
        ...
        for (int lineIndex = 0; lineIndex < lines.Length; lineIndex++)
        {
            ...
            for (int i = 0; i < lineTokens.Count - 1; i++)
            {
                ...
                {
                    LogUnresolvedContext(filePath, lineIndex, functionName, qualifiedPath, localFunctions, includedFiles);
        ...
        foreach (var function in GetFunctionDefinitions(lines))
        {
            ...
            for (int line = bodyStart; line <= bodyEnd; line++)
            {
                ...
                for (int i = 0; i < lineTokens.Count - 1; i++)
                {
                    ...
                    {
                        if (line == function.DefinitionLine && lineTokens[i].Column == function.NameColumn)
                            continue;
    ...
    private void LogUnresolvedContext(
        string currentFilePath,
        int lineIndex,
        string functionName,
        string qualifiedPath,
        ...
        List<IncludedFileScope> includedFiles)
    {
        ...
        var localHit = localFunctions.Contains(functionName);
        var builtInHit = _indexer.BuiltIns.GetBuiltIn(functionName) is not null;

        var includeHits = includedFiles
            .Where(f => f.Functions.Contains(functionName))
            .Select(f => f.Path)
            .Take(8)
            .ToList();

        var qualifiedIncludeHits = string.IsNullOrWhiteSpace(qualifiedPath)
            ? []
            : includedFiles
                .Where(f => PathMatches(f.Path, qualifiedPath) && f.Functions.Contains(functionName))
                .Select(f => f.Path)
                .Take(8)
                .ToList();

        var symbolHits = FindCandidateSymbols(functionName)
            .Select(s => s.FilePath)
    ...
    private static bool PathMatches(string filePath, string scriptPath)
    {
        ...
        if (normalizedScript.EndsWith(".gsc", StringComparison.OrdinalIgnoreCase) ||
            normalizedScript.EndsWith(".gsh", StringComparison.OrdinalIgnoreCase))
        {
            return normalizedFile.EndsWith(normalizedScript, StringComparison.OrdinalIgnoreCase);
        }

        return normalizedFile.EndsWith(normalizedScript + ".gsc", StringComparison.OrdinalIgnoreCase)
            || normalizedFile.EndsWith(normalizedScript + ".gsh", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static bool TryExtractDirectivePath(string line, out string path, bool includeInline = true)
    {
        path = string.Empty;
